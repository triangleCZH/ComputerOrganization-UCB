...
const char* IGNORE_CHARS = " \f\n\r\t\v,()";//ork with strtok,��ҳ ���� �س� ˮƽ�Ʊ� ��ֱ�Ʊ�

/*******************************
* Helper Functions
*******************************/
//these don't need to be implemented by us
...
/* First pass of the assembler. You should implement pass_two() first.
   ...
   be valid. Here are some guidelines:
    ...
    4. Only one instruction should be present per line. You do not need to do
        anything extra to detect this - it should be handled by guideline 3.   fixme: this guideline three is handled by us or not?
...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        // Ignore comments��replace the # with \0 to end the line
        skip_comment(buf);

        // Scan for the instruction name
        char* token = strtok(buf, IGNORE_CHARS);

        if (token != NULL)        //try to figure out if this is wholly a comment line and has no content anymore
        {
            int label_status = add_if_label(input_line, token, byte_offset, symtbl);
                               if (label_status == 1) {
            ...
            if (token != NULL) {
                char* inst = token;//storing the instruction itself
                char** args = (char**)calloc(MAX_ARGS, sizeof(char*));
                if (args == NULL) {
                    allocation_failed();
                }
                int num_args = 0;
                if(parse_args(input_line, args, &num_args)!=0) {
                    ret_code = -1;
                }//use parse arguments to parse arguments :)
                 // Checks to see if there were any errors when writing instructions
                unsigned int lines_written = write_pass_one(output, inst, args, num_args);
                if (lines_written == 0) {
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        int num_args = 0;
        ret_code = parse_args(input_line, args, &num_args);//parsing arguments again
        //add_if_label(input_line, args[0], byte_offset, reltbl);
        if (is_valid_label(args[0])) {
            if(add_to_table(reltbl, args[0], byte_offset) != 0) {
                ret_code = -1;
            };
        }
/* Use translate_inst() to translate the instruction and write to output file.
        ...
        if(translate_inst(output, name, args, num_args, byte_offset, symtbl,reltbl) == -1) {
          ...
          byte_offset += 4;        //increment the relative address
        }
        //int num = 0;        //have not figure out what this line means
        free(args);
    }
