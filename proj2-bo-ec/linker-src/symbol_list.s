...
addr_for_symbol:
	...
	move $s0, $a0
	move $s1, $a1 #saving the function arguments into saved registers

afs_loop:
	lw $t0, 4($s0) #loading the first name in the symbol list to $t0
	move $a0, $t0 #putting the read string into a0 for calling streq
	move $a1, $s1 #putting the string that we are looking for into $a1
	jal streq #testing whether the two strings are equal, putting the result into $v0
	beq $v0, $0, addr_found
	lw $s0, 8($s0)
...
#------------------------------------------------------------------------------
# symbol_for_addr:
# 	beq $a0, $0, symbol_not_found	#if reaching the end of the list
# 	lw $t0, 0($a0) #loading the address (variable) of the new node
# 	beq $t0, $a1, symbol_found #comparing address of the new node and the address we wanted
# 	lw $a0, 8($a0) #if not, traverse to the next element in the list
# 	j symbol_for_addr #and jumping to the beginning of the loop
# symbol_found:
# 	lw $v0, 4($a0)#if it is found, then loading the name of that node into $a0
# 	jr $ra
# symbol_not_found:
# 	li $v0, 0
