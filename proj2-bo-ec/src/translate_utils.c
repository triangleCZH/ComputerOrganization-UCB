...
int translate_num(long int* output, const char* str, long int lower_bound,
    long int upper_bound) {//this may still cannot promise a decimal to a hex
    if (!str) {
        return -1;
    }//if either empty string given or output is NULL (cannot be written), then return -1
    char* endptr;
    long int outvalue = strtol (str, &endptr,0);
    if (*endptr != NULL){
      return -1;
    }
    if (!((outvalue >= lower_bound) && (outvalue <= upper_bound))) {
        return -1;
    }
    *output = outvalue;
    return 0;
 ...
 */
//EC:$zero, $at, $v0, $a0 - $a3, $t0 - $t3, $s0 - $s3, $sp, and $ra
